...
import { useDeleteMealPlanMutation } from "../../redux/api/mealPlanApiSlice";
...
const DeleteMealPlanForm = ({ refetch, onClose, deletingId }: FormProps) => {
  const [deleteApiHandler, { isLoading }] = useDeleteMealPlanMutation();

  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      const res = await deleteApiHandler(deletingId).unwrap();
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col items-center gap-1 mt-6">
      <h2 className="text-xl font-semibold mb-2 ">Delete Meal Plan</h2>
...
export default DeleteMealPlanForm;
